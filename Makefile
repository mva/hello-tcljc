...
JAVA_OPTS=
TCLJC_OPTS=$(JAVA_OPTS) -p $(TCLJC_MDIR) -m tcljc.compiler
...
run-main:
	$(JAVA) -p $(TCLJC_MDIR) --add-modules tcljc.core -cp $(DEST_DIR) $(MAIN_NS).___ $(ARGS)
...
test:
	$(JAVA) --enable-preview -p $(TCLJC_MDIR) --add-modules tcljc.core -cp $(DEST_DIR) $(RUN_TESTS_NS).___
